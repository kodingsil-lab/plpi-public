        ...
        <div class="form-section">
            ...
            <div class="form-grid">
                <label>
                    ...
                    <input type="email" name="mail_from_email" value="<?= esc((string) old('mail_from_email', $row['mail_from_email'] ?? ''), 'attr') ?>" placeholder="plpi@domain.ac.id">
                </label>
                <label>
                    <span>Nama Pengirim</span>
                    <input name="mail_from_name" value="<?= esc((string) old('mail_from_name', $row['mail_from_name'] ?? ''), 'attr') ?>" placeholder="PLPI">
                </label>
                <label>
                    <span>SMTP Host</span>
                    <input name="smtp_host" value="<?= esc((string) old('smtp_host', $row['smtp_host'] ?? ''), 'attr') ?>" placeholder="mail.domain.ac.id">
                </label>
                <label>
                    <span>SMTP Port</span>
                    <input type="number" name="smtp_port" value="<?= esc((string) old('smtp_port', $row['smtp_port'] ?? ''), 'attr') ?>" placeholder="465 / 587">
                </label>
                <label>
                    <span>SMTP Username</span>
                    <input name="smtp_user" value="<?= esc((string) old('smtp_user', $row['smtp_user'] ?? ''), 'attr') ?>" placeholder="plpi@domain.ac.id">
                </label>
                <label>
                    <span>SMTP Password</span>
                    <input type="password" name="smtp_pass" value="" placeholder="<?= ! empty($row['smtp_pass']) ? 'Sudah tersimpan, isi untuk mengganti' : 'Password email resmi' ?>">
                </label>
                <label>
                    <span>Enkripsi</span>
